...
::: tip
您可以使用 `loading` 插槽或 `loadingIcon` 属性自定义您的 loading 状态时显示的图标。
...
可以自定义元素渲染的标签。例如：button、div、路由链接等。
...
| ------- | --------------------------------------------------- |
| default | 默认插槽, 按钮内容                                  |
| loading | 自定义加载图标, 需要设置`loading`属性为true才能使用 |
