...
describe('Alert.vue', () => {
  ...
  it('should support centering text', () => {
    ...
    // class
    const rootNode = wrapper.find('.lf-alert')
  ...
  it('should not render close icon when closable is false', () => {
    ...
    expect(wrapper.find('.lf-alert__close').exists()).toBe(false)
  })
  ...
  it('should toggle visibility when open and close methods are called', async () => {
    ...
    expect(wrapper.find('.lf-alert').attributes().style).toBe('display: none;')
    await wrapper.vm.open()
    expect(wrapper.find('.lf-alert').attributes().style).toBe('')
  })
})
